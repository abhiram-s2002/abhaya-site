  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex">
        <div className="w-screen max-w-full sm:max-w-md bg-[#68043D] text-white shadow-2xl flex flex-col justify-between animate-slide-in-right border-l border-white/20 pb-safe font-semibold">
          
          {/* Drawer Header */}
          <div className="p-4 sm:p-5 border-b border-white/20 flex items-center justify-between bg-[#7A0648]">
          ...
          <div className="flex-1 flex flex-col justify-between overflow-hidden">
              ...
              {cart.length === 0 ? (
                <div className="text-center py-16 space-y-4">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm sm:text-base uppercase tracking-wider font-bold text-white">Your bag is currently empty</p>
                    <p className="text-xs text-white/90 font-medium">Explore our luxury abayas and seasonal collections.</p>
                  </div>
                  <button
                    onClick={() => {
                      setIsCartOpen(false);
                    ...
                    className="btn-primary inline-block mt-3 !bg-white !text-[#7A0648] hover:!bg-white/90 border border-white font-bold"
                  >
                    Explore Boutique
                  </button>
                </div>
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                      <div>
                        <div className="flex justify-between items-start gap-1.5">
                          <h3
                            className="text-xs sm:text-[13px] uppercase tracking-wide font-bold text-white hover:text-white/80 transition-colors cursor-pointer leading-tight truncate"
                            onClick={() => { setIsCartOpen(false); navigateTo('product-detail', item.productId); }}
                            title={item.name}
                          >
                            {item.name}
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-1 text-[10px] uppercase text-white/90 font-semibold">
                          <span className="bg-white/15 text-white px-1.5 py-0.5 rounded-none font-semibold truncate max-w-[200px]">
                            {item.size}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 gap-2">
                        <div className="flex items-center border border-white/30 rounded-none bg-[#580233] text-white shrink-0">
                          <button
                        ...
                        <div className="text-right shrink-0">
                          ...
                          {item.quantity > 1 && (
                            <span className="block text-[9px] text-white/80 tabular-nums font-medium">
            ...
            {cart.length > 0 && (
              <div className="p-4 sm:p-5 border-t border-white/20 bg-[#580233] space-y-3 text-white font-semibold">
                
                {/* Pricing Breakdown */}
                <div className="space-y-1 text-xs text-white uppercase tracking-wide font-medium">
                  <div className="flex justify-between">
                    <span>Subtotal</span>
                    <span className="font-bold text-white">{formatPrice(rawCartSubtotal)}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="flex items-center gap-1.5">
                      ...
                      {userLocation?.country && (
                        <span className="text-[10px] text-white bg-white/20 px-1.5 py-0.5 rounded-none border border-white/25 inline-flex items-center gap-1 font-semibold">
                ...
                <div className="space-y-2 pt-1">
                  <button
                    ...
                    className="w-full py-3.5 bg-white hover:bg-white/90 text-[#7A0648] text-xs uppercase tracking-[0.14em] font-bold transition-all duration-200 flex items-center justify-center gap-2.5 rounded-none active:scale-[0.99] group cursor-pointer shadow-xl border border-white"
                  >
                    <svg
                      className="w-4 h-4 fill-current shrink-0 text-[#7A0648]"
                      viewBox="0 0 24 24"
                    >
